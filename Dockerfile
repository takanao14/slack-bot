...
# BIZ UDGothic is a universal-design face drawn for legibility at small sizes,
# which is what a 32px LED matrix asks for. The proportional cut is used because
# chat prose is mostly kana, punctuation and latin, which the fixed-width cut
# pads out to full-width cells, making the rendered strip longer than it needs
# to be. Both downloads are pinned by checksum; the release archive ships no
# license text, so OFL.txt is fetched from the same tag and travels with the
# font as the OFL requires.
